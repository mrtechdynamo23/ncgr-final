...
// ─── TOP-LEVEL PERMANENT MODULES (ALWAYS VISIBLE) ────────────────
...
export const Sidebar: React.FC<SidebarProps> = ({ collapsed, mobileOpen, onMobileClose }) => {
  ...
  // Collapsible Groups State (default closed/collapsed)
  ...
  useEffect(() => {
    collapsibleGroups.forEach((group) => {
      const hasActiveChild = group.items.some((item) => isRouteActive(item.path));
      if (hasActiveChild) {
  ...
  return (
    <>
      ...
      <aside className={`app-sidebar ${collapsed ? 'collapsed' : ''} ${mobileOpen ? 'mobile-open' : ''}`}>
        ...
        {/* ─── HIERARCHICAL NAVIGATION MATCHING EXACT SPECIFICATION ─── */}
        <nav className="sidebar-nav" role="navigation" aria-label="Portal Navigation">
          {/* Top Permanent Tiles (Executive Control Towers, Team Overview, Command Center) */}
          {topItems.map((item) => {
            const active = isRouteActive(item.path);
            const label = getLabel(item.labelKey, item.defaultLabel);
                  ...
                  {group.items.map((item) => {
                    ...
                    return (
                      <button
                        ...
                        onClick={(e) => handleNavigate(item.path, e)}
                      >
                        <span className="sidebar-sub-icon">{item.icon}</span>
                        {!collapsed && (
